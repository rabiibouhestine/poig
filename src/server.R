...
server <- function(input, output, session) {
  ...
  output$wonder_image <- renderUI({
    tags$img(height = 280, width = "100%", src = current_wonder_image())
  })
  ...
  observeEvent(reset(), {
    ...
    leafletProxy("map", session, data = wow) %>%
      clearMarkers() %>%
    ...
    if (isTRUE(initialise())) {
      leafletProxy("map", session) %>%
        ...
        flyToBounds(
          ...
          lng2 = current_wonder()$Longitude,
          lat2 = current_wonder()$Latitude,
          options = list(duration = 0.5)
        )
